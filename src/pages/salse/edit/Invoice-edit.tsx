...
const InvoiceEdit = () => {
    ...
    useEffect(() => {
        fetch("http://excel_erpnext.localhost:8000/api/resource/Item?fields=%5B%22name%22%2C%22standard_rate%22%2C%22item_name%22%2C%22item_code%22%2C%22description%22%2C%22stock_uom%22%5D&limit=200", {
            headers: {
    ...
    useEffect(() => {
        fetch(`http://excel_erpnext.localhost:8000/api/resource/Sales Invoice/${name}`, {
            headers: {
                Authorization: "token 3a6f30c363313f8:0fbd705429e5816"
            }
            ...
            if (Object.keys(data).length > 0) {
                ...
                setWareHouse(data?.items[0]?.warehouse)
                const formatItem = data?.items.map(el => ({
    ...
    //   limit: 200,
    // })
    const remainingBalance = data?.filter((d) => d.customer === customer)[0]?.excel_remaining_balance;
            ...
            <chakra.form onSubmit={handleSubmitEditInvoice}>
                ...
                <div className="border rounded-md mb-3 p-5">
                    ...
                    <div className="flex gap-5 mb-5">
                        ...
                        <FormControl>
                            <Input type='text' onChange={handleInvoiceInput} name="net_total" placeholder='Net Total' value={form.net_total} />
                        </FormControl>
                        <FormControl>
